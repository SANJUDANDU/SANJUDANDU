# Hi there, I'm Sanjana Dandu! 👋

🎯 **Data Analyst** | SQL • Power BI • Excel  
📍 Mumbai, India  
🔍 Open to new opportunities

---

## 👩‍💻 About Me

I'm a data analyst with a passion for turning raw data into meaningful business insights. I enjoy asking the right questions, building clean data models, and creating dashboards that people can actually use to make decisions.

When I'm not querying databases, you'll find me practising **Taekwondo**, doing **yoga**, creating **mythology content**, or competing at **hackathons**.

---

## 🛠️ Tech Stack

![SQL](https://img.shields.io/badge/SQL-SQL%20Server-blue?style=flat-square&logo=microsoftsqlserver&logoColor=white)
![Power BI](https://img.shields.io/badge/PowerBI-F2C811?style=flat-square&logo=powerbi&logoColor=black)
![Excel](https://img.shields.io/badge/Excel-217346?style=flat-square&logo=microsoftexcel&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white)

---

## 📂 Featured Projects

### 🗄️ [SQL Data Warehouse — Medallion Architecture](https://github.com/SANJUDANDU/SQL-Data-Warehouse-project)
> Built a modern data warehouse using SQL Server with Bronze → Silver → Gold medallion layers. Covers full ETL pipeline design, data modelling, and analytics-ready outputs for BI, SQL reporting, and ML pipelines.

`SQL Server` `ETL` `Data Warehousing` `Star Schema` `Medallion Architecture`

---

### 📊 [Advanced Data Analysis — SQL](https://github.com/SANJUDANDU/Advance-data-analysis)
> End-to-end SQL analysis project covering EDA, cumulative analysis, performance benchmarking, customer segmentation (VIP / Regular / New), and business reporting views with KPIs.

`SQL Server` `Window Functions` `CTEs` `Data Segmentation` `Reporting Views`

---

### ✈️ Flight Operations Dashboard — Power BI
> Interactive Power BI dashboard tracking flight delays, risk categories, airline performance, and ticket pricing across 8 major carriers including IndiGo, Air India, Emirates & Vistara.

`Power BI` `DAX` `Data Modelling` `KPI Cards` `Dashboard Design`

---

## 📈 GitHub Stats

<p align="left">
  <img src="https://github-readme-stats.vercel.app/api?username=SANJUDANDU&show_icons=true&theme=default&hide_border=true" height="150"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=SANJUDANDU&layout=compact&theme=default&hide_border=true" height="150"/>
</p>

---

## 🏆 Highlights

- 🥋 Taekwondo practitioner & yoga enthusiast
- 💻 Hackathon participant — ITS-A-Hack 2.0
- 📖 Mythology content creator
- 💪 Fitness advocate

---

## 📬 Connect with Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Sanjana%20Dandu-0077B5?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/sanjana-dandu)
[![GitHub](https://img.shields.io/badge/GitHub-SANJUDANDU-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/SANJUDANDU)

---

*"Every dataset has a story — I help find it."* 📊
